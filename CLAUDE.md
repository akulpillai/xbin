...
- **`equation_recovery`** ("what does it compute"): `bind_se` (angr symbolic execution + local LLM), `symbolic_regression` (PySR + local LLM), and `pysindy` (Binary Ninja structure analysis + numpy STLSQ sparse regression, from the `submodules/pysyndy` submodule).
