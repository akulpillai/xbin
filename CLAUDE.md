...
- **`equation_recovery`** ("what does it compute"): `bind_se` (angr symbolic execution + local LLM) and `symbolic_regression` (PySR + local LLM).
...
- `score = confidence * BACKEND_WEIGHTS.get(backend_name, 0.5)`; a vouch adds `confidence * weight` to the target's score. `BACKEND_WEIGHTS` is keyed by `backend_name` (the decorator `name=`): `fid` 1.0, `ghidriff` 0.95, `bind_se`/`symbolic_regression` 0.85, `bind_arbiter` 1.0; unknown backends fall back to `0.5`.
